...
- [Specialized Patterns](#specialized-patterns)
  ...
  - [Hex-Encoded String Comparison](#hex-encoded-string-compare)
...
## Pre-Analysis Judgment: File Masquerading and Name Deception

### File Extensions Are Not Trustworthy

**Core principle: Always use the `file` command or magic bytes to determine the file type — never trust the extension.**

Common masquerading techniques:

| Fake Extension | Actual Type | Purpose |
|---------------|-------------|---------|
| `.sh` | ELF binary | Makes people think it's a script, lowering vigilance |
| `.txt` | PE/ELF | Bypasses simple file type filtering |
| `.jpg`/`.png` | Executable or archive | Hidden inside an image |
| `.dll` | Actually a .NET assembly | Misleads analysis direction |
| `.so` | Actually an encrypted payload | Requires decryption first |
...
# Use xxd to check magic bytes
xxd suspicious_file.sh | head -1
# 7f454c46 = ELF magic
```

### File Names Are Not Trustworthy

**"DriverLoader" may not load a driver, "Updater" may not update.**

Common name deceptions:

| Name Implication | Actual Behavior |
|-----------------|-----------------|
| `DriverLoader` | Could be a ptrace injector / process hook |
| `SystemService` | Could be a backdoor / C2 agent |
| `Updater` / `Update` | Could be a dropper / downloader |
| `Helper` / `Assistant` | Could be a privilege escalation tool |
| `lib*.so` | Could be an injection payload |

**When analyzing:**
- Ignore name implications; judge by actual code behavior
- Pay attention to system calls like `mmap`, `ptrace`, `/proc/self/mem`
- If it claims to "load a driver" but has no `insmod`/`init_module` calls, the name is misleading

### Dynamic Analysis as a Supplement When Static Is Insufficient

Pure static analysis can only see the code skeleton. The following scenarios require dynamic analysis:

| Scenario | Recommended Dynamic Method |
|----------|---------------------------|
| Code has decryption/decompression logic | Set breakpoint after decryption, dump plaintext |
| Extensive indirect calls (function pointer tables) | strace/ltrace to trace actual calls |
| Suspected anti-debugging | Use strace first to check for ptrace calls |
| Embedded shellcode/payload | QEMU user-mode emulation |
| Unknown network communication protocol | tcpdump/Wireshark packet capture |

```bash
# strace to trace system calls (key focus)
strace -f -e trace=open,mmap,ptrace,execve,connect ./binary

# ltrace to trace library function calls
ltrace -f ./binary

# QEMU user-mode emulation (no real hardware needed)
qemu-aarch64 -strace ./binary_arm64

# Check for anti-debugging: see if ptrace self-trace is present
strace ./binary 2>&1 | grep ptrace
# If you see ptrace(PTRACE_TRACEME, ...), anti-debugging is present
```

### Common Patterns in Process Injection / Protector Shell Samples

This type of sample (e.g., `LinYuDriverLoader`) typically:

1. **Does not actually load a kernel driver** (requires root, which most scenarios lack)
2. **Actual behavior is process injection**:
   - `ptrace` attach to the target process
   - Read/write target memory via `/proc/<pid>/mem`
   - `mmap` maps shellcode into the target process space
3. **Embeds encrypted payload**:
   - Decrypts a shellcode at runtime
   - The decrypted payload is the actual hook code
4. **Anti-debugging protection**:
   - `ptrace(PTRACE_TRACEME)` self-trace
   - Timing checks (`clock_gettime` before/after comparison)
   - `/proc/self/status` TracerPid check

**Analysis strategy:**
```text
1. file command to confirm the real type
2. strings to look for obvious paths/library names/error messages
3. rabin2 -I to check architecture/compiler/protections
4. Statically find mmap/ptrace/open calls
5. If there is decryption logic → run dynamically and dump after decryption
6. If there is anti-debugging → patch it first or bypass with LD_PRELOAD
```
